...
public class PictureDao {
	...
	public void insert(Connection conn, Picture picture) 
		...
		try {
			...
			("insert into picture (title, name, memo, image, price, rdate, favorite, kind)"
					+ " values(?,?,?,?,?,?,?,?)");
			...
			pstmt.setInt(7, picture.getFavorite());
	...
	public Picture selectById(Connection conn, int pictureId) 
		...
		try {
			...
			if (rs.next()){
				...
				picture.setFavorite(rs.getInt(8));
	...
	public List<Picture> selectTheme(Connection conn, String theme) 
			throws SQLException {
		PreparedStatement pstmt=null; 
		ResultSet rs = null;
		Picture picture = null; 
		List<Picture> pictures = new ArrayList<Picture>();
		String sql = "select * from picture "+theme; // order by favorite desc, order by rdate desc ...
		try {
			pstmt = conn.prepareStatement(sql);
			rs = pstmt.executeQuery();
			while (rs.next()){
	...
	public void updatefav(Connection conn, int pid) 
		...
		PreparedStatement pstmt=null; 
		try {
			pstmt = conn.prepareStatement("update picture set favorite=favorite+1 where pictureId=?");
	...
	public List<Picture> selectKindList(Connection conn, String kind) 
		...
		try {
			pstmt = conn.prepareStatement
					("select * from picture where kind=?");			
	...
	public List<String> getKinds(Connection conn) 
		...
		try {
			pstmt = conn.prepareStatement
					("select kind from picture group by kind ");			
			rs  = pstmt.executeQuery(); 
	...
	public PictureListView getPictureList(Connection conn, int pageNumber)
		...
		try {
			...
			if (pictureTotalCount > 0) {
				...
				endRow = pictureCountPerPage; // 2page -> 5, 5(가져올 때 6, 10까지)
				pictureList = selectListByLimit(conn, firstRow, endRow);
			} else {
				currentPageNumber = 0;
				pictureList = Collections.emptyList();
			}
			plv = new PictureListView(pictureTotalCount, currentPageNumber, pictureList
					, pictureCountPerPage);
		} catch (SQLException e) {
		} finally {
			//JdbcUtil.close(conn);
